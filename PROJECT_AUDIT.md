...
- **Resolution:** The defective browser-client dependency (`await import("@/integrations/supabase/client")`, `advertising.functions.ts:48`) — a genuine server/client boundary violation, confirmed by direct source inspection — has been removed and replaced with the file's own existing `adminClient()` helper, exactly matching the pattern every other function in this file already uses. Before applying the fix, re-investigation attempted to reproduce the originally-reported crash and could not: the original crash mechanism (client construction/query throwing due to `storage: undefined`) was **not reproducible** against the currently-installed `@supabase/supabase-js@^2.110.8`, tested directly via an isolated SSR-module probe (module load + the exact `.from("ad_placements").select(...)` query, executed in a genuine no-`window` Node context) and via both affected endpoints in two real runtimes (Vite Node dev SSR and the actual built Cloudflare Workers bundle under `workerd` — the real deploy target per `vite.config.ts`'s `cloudflare` preset, not "node-server" as originally assumed). Separately confirmed via a read-only production query: no application currently has the `advertising` capability enabled anywhere on the platform, so the capability gate (`advertising.functions.ts:45-46`) has short-circuited every real request before it could ever reach the defective line — meaning the endpoint's observed-good behavior pre-fix was not evidence the defect was harmless, only that it had never been exercised. **No end-to-end test with `advertising` actually enabled for a real application was performed** — doing so would have required a database write, out of scope for this fix. The fix itself was verified via: `npx tsc --noEmit` (only the 4 pre-existing `RT-3` errors, unchanged), `npm run build` (clean), and both endpoints re-tested post-fix showing byte-identical responses to the pre-fix baseline (`{"data":[]}` / `{"data":null}`, both `200`), confirming the advertising-disabled path is unchanged. No database change was required or made — `ad_placements` RLS already permits this read for either client. No `API_CONTRACT.md` or response-shape change — the fix is entirely internal to which Supabase client instance the function uses.
- **Commit:** —
- **Date:** Logged 2026-08-09, resolved 2026-08-13
...
- **Update (2026-08-19, CORE Notification & User Engagement System):** a distinct, more severe bug in this exact code path was found and fixed in the same pass — `sendMessage`'s notification insert used the sender's own RLS-scoped client against a table with no `INSERT` policy for a regular authenticated user, so the notification half of every single send (not just retries) was silently failing, unconditionally. That is now fixed (routed through the new service-role-backed `sendNotification()`). **This finding's own tracked risk is not resolved by that fix** — a genuine network retry still produces a second `messages` row with its own fresh id and therefore its own fresh notification `dedupeKey`, so this finding's recommendation (a client-generated idempotency key) still applies unchanged.
- **Resolution:** —
- **Commit:** —
- **Date:** 2026-07-31
