  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        // succeeded, else resource.id -- same safe fallback already used
        // for entityId elsewhere in this file -- so a webhook retry can
        // never double-notify) and preference/email handling for free.
        await sendNotification({
          userId: ref.user_id,
          appId: ref.app_id,
          category: dependencyRefunded ? "warning" : "premium",
          type: dependencyRefunded ? "warning" : "success",
          dedupeKey: `payment:${insertedPayment?.id ?? resource.id}`,
