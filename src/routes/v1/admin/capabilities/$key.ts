// API_CONTRACT.md §8 -- PATCH /v1/admin/capabilities/{key}.
...
export const Route = createFileRoute("/v1/admin/capabilities/$key")({
  server: {
    handlers: {
      PATCH: withRoute(async ({ request, params }) => {
        ...
        const patch: Partial<CapabilityDefinitionRow> = {};
        ...
        await writeAuditLog({
          userId: admin.userId,
          action: "capability_definition.update",
        ...
        return apiData(toCapability(row as CapabilityDefinitionRow));
      }),
    },
  },
});
