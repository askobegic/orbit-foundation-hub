  ...
  return (
    <main className="min-h-screen bg-[#F7F8FA] px-6 py-10">
      <div className="mx-auto max-w-5xl">
        ...
        <p className="mt-1 text-sm text-gray-500">Premium users eligible for the verified badge.</p>

        <div className="mt-6 space-y-3">
          {q.isLoading && <p className="text-sm text-gray-500">Loading…</p>}
          {!q.isLoading && rows.length === 0 && (
            <p className="rounded-2xl bg-white p-6 text-sm text-gray-500 shadow-sm ring-1 ring-gray-100">No candidates.</p>
          )}
          {rows.map((u) => (
