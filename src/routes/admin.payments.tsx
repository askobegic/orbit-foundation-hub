        ...
        <div className="mt-6 overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-100">
          <table className="min-w-full divide-y divide-gray-100 text-sm">
            <thead className="bg-gray-50 text-left text-xs uppercase tracking-wide text-gray-500">
              <tr>
                <th className="px-4 py-3">{t("admin.payments.date")}</th>
                <th className="px-4 py-3">{t("admin.payments.userId")}</th>
                <th className="px-4 py-3">{t("admin.payments.app")}</th>
                <th className="px-4 py-3">{t("admin.payments.amount")}</th>
                <th className="px-4 py-3">{t("admin.payments.method")}</th>
                <th className="px-4 py-3">{t("admin.payments.status")}</th>
                <th className="px-4 py-3">{t("admin.payments.invoice")}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {q.isLoading && (
                <tr><td className="px-4 py-6 text-center text-gray-500" colSpan={7}>{t("common.loading")}</td></tr>
              )}
              {!q.isLoading && rows.length === 0 && (
                <tr><td className="px-4 py-6 text-center text-gray-500" colSpan={7}>{t("admin.payments.noPayments")}</td></tr>
              )}
              {rows.map((p) => (
                <tr key={p.id}>
                  ...
                  <td className="px-4 py-3">
                    ...
                    }>{t(`admin.common.paymentStatus.${p.status}`, p.status ?? "")}</span>
                  </td>
                  <td className="px-4 py-3">
                    {p.invoice_url ? (
                      <a href={p.invoice_url} target="_blank" rel="noreferrer" className="text-[#1D6BF3] hover:underline">{t("admin.payments.open")}</a>
                    ) : "—"}
                  </td>
                </tr>
              ))}
