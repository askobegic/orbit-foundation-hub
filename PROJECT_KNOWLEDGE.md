...
**Status:** Authoritative. **Consolidation date:** 2026-08-16 — this pass folded in the Commercial Products/Entitlement architecture (Listing, Sponsored, benefit-only purchases, dependency + refund handling, the `/v1/me/entitlements` endpoint) and the per-application Premium Sale/Benefits distinction, both implemented earlier in the same work stream as a chain of audits but not yet reflected here, plus a server-side Application Visibility enforcement fix (payment/webhook layer) found and closed during that same audit chain. Nothing pre-existing was rewritten or reinterpreted — this is additive consolidation, not a redesign. **Every CORE priority shipped since (17–22: Special Offers & Public Coupons, Commercial Products' Application Premium Benefits & Entitlement API, the CORE Notification & User Engagement System, Universal Premium-Locked Content, User Engagement & Dashboard Actions, and the Universal Affiliate System & Points Purchase) added its own self-contained section at the time it shipped; this file was re-audited section-by-section against the actual current code/database (not the reverse) during a documentation synchronization pass on 2026-08-21, which found and corrected one stale cross-reference (the Premium Status Resolver's `source` values — see Premium Status Resolver, below) and otherwise confirmed the existing sections still accurately describe the implementation.** This document remains the single CORE architecture specification for this repository — see Documentation Rules in `CLAUDE.md` for why a second "master specification" file is deliberately not created alongside it.
...
**Premium Status Resolver (`src/lib/premium.server.ts`, Priority 8.7)** — the server-only, bulk-capable sibling to the client-callable `hasAnyActivePremium()` above. Every admin/bulk consumer (`adminListUsers`, `adminOverviewStats`, `adminSendNotification`, `adminListVerificationRequests` in `admin.functions.ts`, and any future `/v1` API endpoint) must resolve Premium status through `resolvePremiumStatusBulk(supabaseAdmin, userIds?)` / `resolvePremiumStatus(supabaseAdmin, userId)` instead of re-querying `subscriptions` directly — the exact "two places compute the same answer differently" pattern `CLAUDE.md` calls a defect (see `PROJECT_AUDIT.md` → `A-5`). Unlike the boolean `hasAnyActivePremium()`, this exposes the complete state — `{ active, source: "subscription" | "trial" | "entitlement" | null, expiresAt }` (`entitlement` added in Priority 15C, see Entitlements below — a third, independent source, not a replacement for the first two) — since a bulk/admin consumer (and a future API client) needs to know not just whether someone is Premium but why and until when. Three queries total regardless of how many users are resolved (`subscriptions`, `promotional_trials`, `entitlements`), never N+1. When a user has more than one active source, a subscription is reported first, then a trial, then an entitlement — the paid/dedicated source always wins as the reported source (see Promotional Trial below for why a subscription and a trial can never conflict).
...
- **Duplicate protection is a first-class column, not per-call-site logic.** `notifications.dedupe_key` + a unique index on `(user_id, dedupe_key)` gives every call site atomic, DB-enforced idempotency via `upsert(..., {onConflict: 'user_id,dedupe_key', ignoreDuplicates: true})` — the same pattern `user_achievements`/`user_reward_milestones` already used, applied to `notifications` itself for the first time. Deliberately **not** a partial index (`WHERE dedupe_key IS NOT NULL`, the first attempt) — live verification found Postgres's `ON CONFLICT` inference cannot match a partial index unless its exact `WHERE` predicate is repeated in the conflict clause, which PostgREST's `upsert({onConflict})` has no way to express, so every dedup-requesting call would have silently failed. A plain unique index is semantically identical for this purpose (Postgres never treats two `NULL`s as colliding), so the majority of notifications, which never request dedup, are unaffected. `sendNotification()`'s return value (`created: boolean`) tells the caller whether a row was actually inserted or silently skipped as a duplicate.
- **A real production bug — distinct from the already-tracked `PROJECT_AUDIT.md` → `MSG-1` (a network-retry could duplicate a message *and* its notification; still open, unaffected by this fix) — was found and fixed while building this: new-message notifications weren't being created at all.** `sendMessage` (`message.functions.ts`) inserted via the sender's own RLS-scoped client, but `notifications` has no `INSERT` policy for a regular authenticated user, only "own row" `SELECT`/`UPDATE` — live production verification confirmed the insert was rejected by RLS on every single call, with the error swallowed by a bare `console.error`. Routing this call site through `sendNotification()` (service-role-backed) fixes this outright-failure case; it does not by itself close `MSG-1`, since a genuine retry still produces a second `messages` row with its own fresh id (and therefore its own fresh `dedupeKey`), which `MSG-1`'s recommendation (a client-generated idempotency key) would still be needed to prevent.
- **Email delivery (Resend)** — `src/lib/email.server.ts`, a plain `fetch` call to Resend's REST API (`RESEND_API_KEY`/`RESEND_FROM_EMAIL`), no new npm dependency. Supersedes the earlier "Email was explicitly not built" state (see Admin ↔ User Communication, below) — the provider decision was explicitly surfaced to and made by the project owner, not assumed. Fails gracefully: a missing key or a failed request never throws, `notifications.email_status` (`not_applicable`/`pending`/`sent`/`failed`) and `email_error` record the outcome on the notification row itself, and the in-app notification is always created regardless of email deliverability. **Not yet configured in this environment** — `RESEND_API_KEY`/`RESEND_FROM_EMAIL` are documented (`DEPLOYMENT.md`) but unset, so email delivery is built and production-capable, not yet operational; in-app notifications are fully functional independent of this.
...
A generic, CORE-wide mechanism for "this field/content requires Premium (or a specific Application Premium Benefit) to reveal its real value" — the pattern Contact Actions (Profile Card & Messaging System, above) already needed, now factored out into a reusable primitive instead of living only inline in one feature.

- **Not a new eligibility system.** `isContentUnlocked(supabaseAdmin, userId, requirements)` (`src/lib/content-lock.server.ts`) is a thin OR-combinator over the two eligibility sources that already exist: Global Premium (`resolvePremiumStatus()`) and the generic Entitlements/Application Premium Benefit layer (`hasActiveEntitlement()`, Entitlements above). CORE evaluates whichever requirements the caller passes in (`{ type: "global_premium" }` and/or `{ type: "entitlement", benefitType, appId }`) — it never decides *which* content needs Premium; that stays entirely the calling application's own decision, expressed by which requirements it asks for. No application id or name is ever branched on inside this file.
- **The security rule this exists to enforce: a Standard caller's response must never contain the protected value, full stop — not sent-then-hidden with CSS/JS.** `resolveLockableField(rawValue, unlocked)` produces `{ exists, locked, value }`: `value` is populated only when `unlocked` is true; when it isn't, `value` is always `null`, regardless of whether the underlying content exists. `exists` is a separate, deliberately non-sensitive flag — the UI needs to know a field *exists* (to render "WhatsApp 🔒 Premium" instead of hiding the row, or "—" when there's genuinely nothing set) without ever learning what the value is.
- **First (and, at the time of writing, only) consumer: Contact Actions on the public profile page**, which is also the bug this capability fixed — see `PROJECT_AUDIT.md` → `SE-20`. `src/routes/u.$username.tsx` used to fetch `profiles_public`/`premium_profiles_public` directly from the browser and hand the full row (real WhatsApp/phone/email/website values, every visitor, regardless of their own Premium status) to `ProfileCard.tsx`, which then decided client-side which *label* to show — the real values were already present in the network response and component state for a Standard visitor, readable without any client manipulation. `src/lib/profile.functions.ts`'s `getPublicProfileForViewer` replaces that fetch: a public (no `requireSupabaseAuth`) server function that resolves the caller's own identity from their verified Supabase session token (never a client-supplied id — see the file's own comment for why), validates `appId` against a real registered application, and returns a bundle where every Contact Actions field is already a `{ exists, locked, value }` triple. `ProfileCard.tsx` now renders from that bundle instead of computing eligibility itself; `ContactActionButton` takes a `locked` flag and a `value` that is only ever non-null when unlocked.
- **`/v1/profiles/$username.ts` (the public REST API) already implemented this correctly and was left untouched** — its own `contactActions` shape (built inline, pre-dating this capability) collapses "doesn't exist" and "locked" into the same `null`, which is a defensible, already-shipped, slightly more conservative design for that specific endpoint; changing its public response shape was out of scope for closing a client-side bug that endpoint never had.
- **Future consumers** (a future application's own Premium-only business field) call `isContentUnlocked()`/`resolveLockableField()` directly from their own server function — CORE does not need a new endpoint per field, and does not need to know the business meaning of what it's gating.
...
## User Engagement & Dashboard Actions

A generic, CORE-wide layer for the central Dashboard to show relevant offers, actionable opportunities, pending/completion prompts, discovery opportunities, resource references, and recent activity — without CORE ever implementing an individual application's business logic. CORE understands only generic concepts (Application, Offer, Action, Activity, Resource, Target, Eligibility, Status, CTA, Destination); which specific content is Premium-only, which field requires completion, or what a "Shop" actually is remains entirely the calling application's own decision.

- **Deliberately separate from Dashboard Offers (Priority 17, `dashboard_offers`).** That table is a commercial discount promotion — it requires a real purchasable product (`subscription_plan` or `ad_placement_price`) and a discount, and is dropped entirely if the referenced product doesn't resolve. This layer's `dashboard_actions` has no product/discount concept at all: a generic title/CTA/destination, optionally scoped to an application, with no purchase involved. Loosening `dashboard_offers`' NOT NULL product/discount columns to also fit this need would have been exactly the "redesign working architecture to fit an unrelated concept" this feature must avoid — so this is genuinely new, small schema, not a reuse of that table. Both tables do reuse the same `offer_segments` registry for audience targeting and the same `resolveAudience()`/notification pipeline (`sendNotification`/`sendBulkNotifications`) — targeting and notifications are not duplicated a second time.
- **`dashboard_actions`** (`src/lib/dashboard-actions.functions.ts`) — the single table backing both "Admin Dashboard Offers/Actions" and "Application-Provided Actions." There is no per-application write credential anywhere in this codebase (every other admin-extensible, per-application registry — `event_rules`, `ad_channels`, `dashboard_widget_settings` — is admin-managed, not application-pushed), so an application "exposing" an action to CORE means an admin configures a row with `app_id` set to that application; `app_id IS NULL` means a platform-wide/CORE action. `action_type` (`offer` / `action` / `complete_task` / `discovery`) is a display-grouping key only, not a business-logic branch — it lets the Dashboard explain *why* something is being shown (spec section 11), nothing more. Targeting reuses the exact `target_type`/`target_segment`/`target_user_id` discriminator shape `dashboard_offers` already established (segment-targeted vs. individual). `requires_missing_resource_type` is the one minimal generic eligibility hook beyond audience segment: when set, the action is only shown to a user with no matching `resource_references` row for that action's `app_id` — resolving the "Open your Shop" (only shown to a user with no Shop) example generically, without CORE ever knowing what "shop" means. `starts_at`/`ends_at` are nullable ("where appropriate"), unlike `dashboard_offers`' mandatory promotion window. `destination` accepts either an internal relative path or an absolute http(s) URL to a different CORE-connected application's own domain (`isSafeDashboardDestination()`, rejecting non-http(s) schemes and scheme-relative `//host/...` — the same "validate before storage" rule `PROJECT_AUDIT.md` → `CO-1` established, extended here since Dashboard Actions must be able to point at another application's own deployment, unlike `notifications.target_path`'s narrower CORE-internal-only constraint).
- **`resource_references`** — a generic pointer to a resource a user owns in a connected application's own database (a Shop, a Business, an Event…). CORE stores only `resource_type`/`label`/`status`/`destination`, never the underlying business data. Written exclusively by `service_role` — either an admin (support/testing, `/admin/dashboard-actions`) or the application itself via `PUT /v1/me/resources/{resourceType}` (`API_CONTRACT.md` §9), authenticated exactly like `POST /v1/events` (the caller's own verified JWT `sub`/`azp`, never a body-supplied user/app id). One row per `(user, app, resource_type)` — a deliberate V1 simplification (every current spec example is inherently singular: "My Shop," "My Business"); a future application needing multiple resources of the same type per user would need this constraint relaxed, not built ahead of an actual need.
- **Recent Activity is pure reuse, zero new backend.** The Dashboard's compact "Recent Activity" panel (`RecentActivity.tsx`) renders a short slice of `getMyActivityDashboard()`'s existing `recentActivity` field (Cross-App User Activity Dashboard, Phase E, above) — already the complete, meaningful (`points > 0`, never a page-view/click log), per-user activity feed. No new event storage, no new query.
- **Dashboard Widget Modularity gates both new sections** (Priority 8.2, reused, not a new visibility mechanism) — two new registry rows, `dashboard_actions` (My Resources + For You/Complete/New/Discover) and `recent_activity`, both admin-toggleable globally or per application exactly like every other Dashboard section.
- **Notifications reuse the existing pipeline, never a second one.** Publishing a brand-new, already-enabled `dashboard_actions` row (or an explicit disabled→enabled transition) notifies its resolved audience through `sendNotification()`/`sendBulkNotifications()`, mirroring `dashboard_offers`' `notifyOfferPublished()` exactly — `dedupeKey: dashboard_action:${id}` scoped to the row's lifetime, so re-enabling doesn't re-notify. Category is the existing `offer` value for `action_type='offer'`, `information` for everything else — no new notification category was added.
- **Security/isolation:** `dashboard_actions` RLS lets an authenticated user see any enabled, non-archived segment-targeted row or their own individual row (segment matching, the date window, and the resource-presence gate are all re-checked server-side in `resolveMyDashboardActions()` — the same "RLS is the safe boundary, business filtering is a server function" split `dashboard_offers` already uses); `resource_references` RLS restricts SELECT to the owning user's own rows only, with no authenticated INSERT/UPDATE policy at all (service-role-only writes, the same pattern `conversations`/`ad_campaigns` already use for business-rule-gated tables). Both tables carry the same broad `anon`/`authenticated` table-level GRANT every table in this schema already receives from a pre-existing database default — RLS, not the GRANT, is the actual enforcement boundary here, confirmed empirically (an authenticated non-admin `INSERT` into `resource_references` is rejected with `42501`).
- **Affiliate is explicitly out of scope and untouched by this layer** — no attribution, tracking, commission, or payout concept exists anywhere in `dashboard_actions`/`resource_references`; a future Affiliate module would consume this same generic Action/Resource mechanism (e.g. an affiliate program surfaced as a `dashboard_actions` row) without requiring any change here.
- **Admin UI:** a new `/admin/dashboard-actions` page (Global Actions, Individual Actions, Resource References as three Card sections), matching the existing precedent of `/admin/offers-coupons`/`/admin/events` each having their own page — reuses the existing admin authorization (`assertAdmin`) and audit logging (`writeAuditLog`), no new admin role.

## Universal Affiliate System

A central CORE Affiliate System through which users promote only Admin-approved CORE or connected-application products/services and earn commission from valid, attributed conversions. CORE owns the Affiliate infrastructure (eligibility, tracking, attribution, commission ledger, status); the originating application (or CORE itself, for Premium/Advertising/Points Packages) remains owner of its own product/service, order, payment, fulfillment, and refund logic — CORE never becomes owner of an application's product data.

- **Global switch** (`affiliate_config`, key/value, same shape as `reward_config`/`ad_config`) — `enabled` (OFF by default), plus platform-wide defaults `default_attribution_window_days` (30), `default_return_period_days` (14), `payout_threshold_eur` (50), `payout_cycle_day_of_month` (informational). **Per-application switch reuses Capabilities (Priority 8.1) exactly** — a new `affiliate` capability key, toggled per application via the existing `application_capabilities` mechanism, not a second per-app toggle table. Final eligibility for an application-sourced offer requires: Global Affiliate ON AND the application's `affiliate` capability enabled AND the offer itself enabled/non-archived AND (for earning) the Affiliate's own status is `active`.
- **`affiliate_offers`** — the Admin-approved catalog, OFF by default per offer (unlike every other CORE catalog in this codebase, which defaults enabled). `source_type` (`core`/`application`) + `source_app_id` (required only for `application`) + `source_product_type`/`source_product_id` identify what's being promoted — for `core` offers, `source_product_type` is `subscription_plan`/`ad_placement_price`/`points_package`, resolved server-side exactly like `offers.functions.ts`'s existing polymorphic `resolveProduct()` pattern conceptually (Affiliate doesn't call that function directly, since it needs no price lookup — commission is computed from the *reported* eligible amount, not a re-fetched price); for `application` offers, `source_product_id` is an opaque string CORE never resolves — the application reports its own eligible amount at conversion time. Commission is `percent` (rate 0–100) or `fixed` (a flat currency amount) — never assumed to be a single global rate. `attribution_window_days`/`return_period_days` are nullable per-offer overrides of the global defaults.
- **Commission snapshot (spec-mandatory).** Every `affiliate_conversions` row freezes `eligible_amount`, `currency`, `commission_type`, `commission_rate`/`commission_fixed_amount`, the computed `commission_amount`, and `return_period_days` at conversion time — a later Admin change to the offer's commission rate never retroactively changes an existing conversion.
- **Attribution: last valid click wins, application-cooperative for cross-domain destinations.** `affiliate_links` (one per `(affiliate, offer)`, a non-guessable `code`, no internal id ever exposed) → `affiliate_clicks` (minimal: which link, when — no IP/user-agent/surveillance data). The public redirect `/r/{code}` (`src/routes/r.$code.tsx`) records the click, captures a **last-touch** local marker (`src/lib/affiliate-tracking.ts` — deliberately not the same mechanic as `referral.ts`'s first-touch invite-referral capture, a different semantic), and redirects to the offer's destination with `?core_ref={code}` appended as a best-effort signal a cooperating external application's own page can also read (CORE cannot force a foreign domain to honor a cookie it never set — an explicit, documented limit, not a silent gap). At conversion time, the most recent click on the resolved link is checked against the attribution window; self-referral (affiliate promoting to themselves) is rejected outright.
- **CORE-native purchases (Premium, Points Packages) bridge the code through `affiliate_pending_attributions`, not the signed payment reference.** This codebase has no dynamic Checkout Session creation (static, admin-configured Stripe/PayPal Payment Links only — `payment-reference.server.ts`), so there is no channel to carry an affiliate code through the payment provider itself. `registerCheckoutAttribution` (called by `pricing.tsx`/`BuyPoints.tsx` right before creating the payment reference, only when a stored code exists) records a small pending-attribution row; the webhook (`recordAffiliateConversionForCorePurchase`) looks up the most recent one for `(user, sourceProductType, sourceProductId)` at fulfillment time and consumes it either way. Application-reported conversions (`POST /v1/affiliate/conversions`) don't need this bridge at all — the application supplies its own code directly, synchronously with its own confirmed transaction.
- **Idempotency is a real UNIQUE constraint**, not just application logic: `affiliate_conversions.transaction_ref` is UNIQUE — the same originating transaction (a `payments.id` for CORE-native purchases, or an application's own order id, namespaced `{appId}:{ref}` by the `/v1` endpoint so two applications can never collide on the same ref string) can produce at most one conversion, ever.
- **Lifecycle: PENDING → (computed) APPROVED → PAID, or → REVERSED.** `status` only ever stores `pending`/`paid`/`reversed` — "Approved" is a **computed label**, never a stored value: `pending` becomes effectively "approved" once `return_period_days` has elapsed since `created_at`, resolved live wherever it matters (the payout sweep, the Affiliate/Admin dashboards) — the same "deterministic, time-based, no cron needed" philosophy this codebase already applies to entitlements/trials/campaigns/Dashboard Actions windows, extended here rather than inventing a stored-state transition and the sweep job that would otherwise be needed just to flip it. A refund/return/chargeback (`reverseAffiliateConversion`, called automatically from both payment webhooks' existing refund handling for CORE-native purchases, and from `POST /v1/affiliate/conversions/{ref}/reverse` for application-reported ones) reverses a not-yet-paid conversion in place (nothing was ever paid out for it to misstate); an **already-paid** conversion is never mutated — a new negative-amount conversion row is created instead (`reversed_conversion_id`), the same append-only "a reversal is always a new row" convention `reward_ledger`'s `reversePaymentPoints()` already established.
- **Automatic payout is genuinely automatic *scheduling*, honestly *not* automatic *money movement*.** `runAffiliatePayoutSweep()` (`POST /v1/system/affiliate-payout-sweep`, the same external-scheduler + shared-secret pattern as `POST /v1/system/inactivity-sweep` — this codebase has no cron infrastructure) aggregates every Affiliate's approved, not-yet-batched balance and creates one `affiliate_payouts` row per Affiliate that meets `payout_threshold_eur`, linking the included conversions to it (idempotent — a conversion already linked to a payout is never re-included; below-threshold balances carry forward automatically, simply by not yet having a payout). **No automated external payout provider is integrated in this codebase** (no Stripe Connect payouts, no PayPal Payouts API) — this is documented, not silently implied as fully automated (spec's own explicit requirement): an Admin marks a payout `paid` manually (`adminMarkAffiliatePayoutPaid`), recording the real external reference (bank transfer, manual PayPal Payout, etc.) once actually sent.
- **Security/isolation:** an Affiliate sees only their own `affiliates`/`affiliate_conversions`/`affiliate_payouts`/`affiliate_links` rows (RLS, `auth.uid()`-scoped); `affiliate_offers` (enabled/non-archived only) and `points_packages` (active only) are the public catalogs; `affiliate_clicks`/`affiliate_pending_attributions` have no direct client SELECT/INSERT policy at all (service-role only, written exclusively through server functions/webhooks that re-verify the caller). Every table carries the same broad `anon`/`authenticated` table-level GRANT this schema's pre-existing default-privilege configuration applies to every table (`PROJECT_AUDIT.md`'s existing PR11-era findings already document this) — RLS, not the GRANT, is the actual enforcement boundary, confirmed empirically for this task (an unrelated authenticated user reading another Affiliate's `affiliates`/`affiliate_conversions` rows returns zero rows).
- **Reuses rather than duplicates:** `sendNotification`/`sendBulkNotifications` (a new `affiliate` notification category, alongside the existing vocabulary — no second notification system); the Universal Event Engine (`recordEvent`, application-attributed conversions only — a CORE-native conversion with no application context has no event to attribute it to, and isn't forced into one); `resource_references` (Priority 21) is what makes "has this user become an Affiliate" work through the *existing, unmodified* Dashboard Actions "Postani Affiliate" prompt — `becomeAffiliate()` upserts an `affiliate_account` resource (app_id **null**, a CORE-wide resource — `resource_references.app_id` was loosened from NOT NULL to nullable for exactly this case, safe since that table had no real production rows yet), which both stops the "Postani Affiliate" prompt (`requires_missing_resource_type` no longer matches) and starts an "Affiliate — active" card appearing in the existing "My Resources" section, with no second "Moj Affiliate" prompt ever needed.
- **Explicitly not implemented, per instruction:** multilevel/tiered affiliate structures, Affiliate-to-Affiliate referral, any Points → cash mechanism, and a general-purpose wallet — none of this system's tables or functions assume or require any of them.
- **UI:** `/dashboard/affiliate` (join flow, stats, catalog with per-offer "Promoviraj"/link-copy, recent conversions) and `/admin/affiliate` (global config, offers, Affiliates with suspend/disable, conversions with manual reverse, payouts with mark-paid) — both new pages, matching the existing Card-based admin pattern (`/admin/offers-coupons`, `/admin/dashboard-actions`).

## Points Purchase

Extends the *existing* CORE Rewards & Loyalty ledger (Priority 8.3/12/16) so a user can optionally buy Points with real money — there is still exactly **one** Points balance/ledger (`reward_ledger`, unchanged schema and unchanged balance computation); this is not a second Points system.

- **`points_packages`** — the purchasable catalog, deliberately mirroring `subscription_plans`' existing shape and conventions: nullable `app_id` (a package is CORE-native, not owned by any one application, but may be branded/sold through a specific application's checkout context — the same reasoning `subscription_plans.app_id` already uses), static `stripe_payment_link`/`paypal_payment_link` (this codebase has no dynamic Checkout Session creation anywhere), `points_amount` + separate `bonus_points`, optional `valid_from`/`valid_until`/`purchase_limit_per_user`.
- **Global switch:** a new `buy_points_enabled` key in the *existing* `reward_config` table (not a new config table) — OFF by default.
- **Purchased Points are just another `reward_ledger` row.** A new `origin` value, `points_purchase` (additive `CHECK` constraint change, alongside the existing `core`/`application`/`api`/`n8n`/`manual_admin`/`system`/`refund_reversal`), written by `grantPurchasedPoints()` (`rewards.server.ts`) — deliberately **not** routed through `grantRewardAction()`, since that function's whole design is "look up a configured value for a known action key" (`reward_action_rules`), while a Points Purchase's amount is decided by which package was bought, already resolved server-side from `points_packages` before the ledger write (never a client-supplied value). Because the granted amount lands in the same `reward_ledger` with the same non-negative guarantee as every other origin, **every existing balance computation, `redeemReward`/`redeem_reward_atomic` spend logic, and Points → Premium/Reward conversion path already works for purchased Points with zero code changes** — this is what "do not create a second Points balance/system" actually means in practice here, not just a policy statement.
- **Refund reversal reuses `reversePaymentPoints()` unmodified.** `grantPurchasedPoints()` writes with `resource_type: "payment"`/`resource_id: paymentId` — the exact convention `reversePaymentPoints()` (Priority 16, already used by Premium/Advertising purchase refunds) already scans for. A Points Purchase refund therefore reverses automatically, proportionally to the refunded amount, through the *same* webhook refund-handling code path that already exists — no new reversal logic was written for this task.
- **Purchase flow mirrors the existing subscription-plan purchase exactly**, not a new payment mechanism: `signPointsPackageReference`/`verifyPointsPackageReference` (`payment-reference.server.ts`) is a new tagged-reference variant, checked early in both Stripe/PayPal webhooks (the same position/pattern as the existing campaign-purchase and coupon-purchase branches) — `resolvePointsPackagePurchase()` (`points-purchase.server.ts`) re-verifies the package is active/in-window and the paid amount/currency match the package's *real*, server-side price before granting anything; the client never decides how many Points are granted. `payments` gained one nullable column, `points_package_id` (FK `points_packages`), mirroring the existing `subscription_id`/`campaign_id` columns exactly — which of the three (if any) a given payment row is for.
- **UI:** a "Buy Points" section directly on the existing `/dashboard/rewards` page (`BuyPoints.tsx`) — not a new Dashboard page — plus an admin-editable "Need more Points?" Dashboard Action (the existing `dashboard_actions` mechanism, Priority 21, reused, not a new one) admins keep in sync with the `buy_points_enabled` switch manually, the same way every other Dashboard Action is admin-toggled. Admin management is a new, small `/admin/points-packages` page (packages + the Buy Points switch + a read-only recent-purchases view over the existing `reward_ledger`) rather than growing the already-large `/admin/rewards` page further — the same precedent `/admin/offers-coupons` already set by splitting off from it.
- **Points → cash was explicitly not implemented** — no redemption path in this codebase converts Points into a payable/withdrawable amount; Points only ever move toward existing Rewards/Premium benefits, never the reverse.
