...
- **Purchase flow mirrors the existing subscription-plan purchase exactly**, not a new payment mechanism: `signPointsPackageReference`/`verifyPointsPackageReference` (`payment-reference.server.ts`) is a new tagged-reference variant, checked early in both Stripe/PayPal webhooks (the same position/pattern as the existing campaign-purchase and coupon-purchase branches) — `resolvePointsPackagePurchase()` (`points-purchase.server.ts`) re-verifies the package is active/in-window and the paid amount/currency match the package's *real*, server-side price before granting anything; the client never decides how many Points are granted.
