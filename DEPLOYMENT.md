...
- **Affiliate monthly payout sweep (optional, Universal Affiliate System)**: same shared-secret pattern and same `SYSTEM_CRON_SECRET` — no separate secret to generate. Point a monthly (or more frequent — the sweep is idempotent, safe to run as often as desired) scheduled job at `POST https://yourdomain.com/v1/system/affiliate-payout-sweep` with header `X-Cron-Secret: <SYSTEM_CRON_SECRET>`. This only *batches* eligible commissions above the configured threshold into a `pending` payout record — it does not transfer any money. **No automated external payout provider (Stripe Connect payouts, PayPal Payouts API, etc.) is integrated in this codebase.** An Admin must still mark each payout `paid` manually from `/admin/affiliate` once the actual transfer (bank transfer, manual PayPal Payout, etc.) has been sent, recording that transfer's real reference. Do not represent Affiliate payouts as fully automated in any user- or Admin-facing communication until a real payout provider is integrated.
- **Affiliate / Points Purchase payment configuration**: both reuse the *existing* Stripe/PayPal static Payment Link + webhook infrastructure above — no new payment processor, no new webhook endpoint, no new provider credentials. A Points Package or an Affiliate-eligible CORE product still needs its own `stripe_payment_link`/`paypal_payment_link` configured by the Admin (`/admin/points-packages`, `/admin/applications`) exactly like every other purchasable CORE product.
