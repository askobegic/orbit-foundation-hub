...
**Status:** Frozen design specification, all open questions resolved (Priority 8.9 — see §22). **No `/v1` endpoint is implemented.** No route has been created, no HTTP handler exists. This document is the complete, versioned public REST contract between CORE and every application built on top of it (BosniaFans, Svadba, Ticketaria, Muzika.ba, Gradovi.ba, Bosanci.info, and every future application) — the permanent reference the implementation phase (`CLAUDE.md` → Priority 8.11) builds against, and the thing every future application integrates with instead of touching CORE's database directly.
